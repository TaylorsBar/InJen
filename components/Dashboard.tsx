...
const InfoCard: React.FC<{ icon: React.ReactNode, value: string | number, label: string, colorClass?: string }> = ({ icon, value, label, colorClass = 'text-cyan-300' }) => (
...
export const Dashboard: React.FC<DashboardProps> = ({ telemetryData, livePath, isCoachEnabled, isCoachSpeaking, onToggleCoach, lapData }) => {
  ...
  const fusionTierInfo = {
    [FusionTier.TIER_1_FULL_FIDELITY]: { text: 'GNSS', color: 'text-cyan-300', sub: 'Full Fidelity' },
    [FusionTier.TIER_3_DEAD_RECKONING]: { text: 'IMU', color: 'text-yellow-400', sub: 'Dead Reckoning' },
    [FusionTier.TIER_4_INITIALIZING]: { text: 'INIT', color: 'text-gray-500', sub: 'Awaiting Lock' },
  };
  ...
  return (
    <div className="flex flex-col space-y-2 h-full justify-between">
      ...
      <div className="grid grid-cols-4 gap-2 text-center text-sm">
        ...
        <InfoCard icon={<FusionIcon className={`w-4 h-4 mb-1 ${currentTier.color}`}/>} value={currentTier.text} label={currentTier.sub} colorClass={currentTier.color} />
      </div>
